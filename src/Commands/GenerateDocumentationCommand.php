...
class GenerateDocumentationCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($publishedVersions as $version) {
            ...
            if ($this->createVersionLanding(base_path($versionDirectory))) {
                $this->line(config('wiki.docs.landing').'.md created under '.$versionDirectory);
            } else {
                $this->line('<info>'.config('wiki.docs.landing').'.md</info> for <info>v'.$version.'</info> already exists.');
            }
    ...
    {
        $landingPath = $versionDirectory.'/'.config('wiki.docs.landing').'.md';
    ...
    protected function generateIndexContent($stub)
    {
        $content = str_replace(
            '{{LANDING}}',
            ucwords(config('wiki.docs.landing')),
            $stub
        );

        $content = str_replace(
            '{{LANDINGSMALL}}',
            trim(config('wiki.docs.landing'), '/'),
            $content
        );
