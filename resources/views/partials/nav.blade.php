           ...
           {{-- <larecipe-button tag="a" href="https://github.com/saleem-hadad/larecipe" target="__blank" type="black" class="mx-2 px-4">
            ...
            <larecipe-dropdown>
                <larecipe-button type="primary" class="flex">
                    {{ $currentRole }} <i class="mx-1 fa fa-angle-down"></i>
                </larecipe-button>
                    ...
                    <ul class="list-reset">
                        @foreach ($roles as $role)
                            <li class="py-2 hover:bg-grey-lightest">
                                <a class="px-6 text-grey-darkest whitespace-no-wrap" href="{{ route('wiki.show', ['version' => $currentVersion, 'role' => $role, 'page' => $currentSection]) }}">{{ $role }}</a>
                            </li>
                        @endforeach
                    </ul>
                </template>
